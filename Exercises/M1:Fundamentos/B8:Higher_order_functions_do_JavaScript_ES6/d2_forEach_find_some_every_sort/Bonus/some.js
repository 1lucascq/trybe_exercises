...
const verifyAge = array => {
  console.log(array)
  const chocoTrybe = "ChocoTrybe, melhor que console.log";
  const trybeer = "Beba Trybeer, suave como um log de erro vazio";
  const whichAdd = (array) => {
    if (array.some(elemento => elemento < 18)) {
    if (array === true) {
      return chocoTrybe;
    } else {
      return trybeer;
...
console.log(verifyAge(arrayIdades2))
