...
const getAll = async () => {
  const [authors] = await connection.execute(
    'SELECT id, first_name, middle_name, last_name FROM model_example.new_authors;'
  );
...
const findById = async (id) => {
  const query =
    'SELECT id, first_name, middle_name, last_name FROM model_example.new_authors WHERE id = ?';
...
const createAuthor = async (firstName, middleName, lastName) => {
  const [author] = await connection.execute(
    'INSERT INTO model_example.new_authors (first_name, middle_name, last_name) VALUES (?, ?, ?)',
